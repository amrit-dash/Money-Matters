...
| `parse_jobs/{auto}` | auto | `ingestSms` CF creates `pending`; app sets `done`/`failed`; `retryStuckParseJobs` may set `stuckAt` | drain + pipeline | `rawIngestId`, `status`, `rulesVersion`, `error`, `updatedAt`, `stuckAt?` |
| `transactions/{txId}` | same as raw ingest id | `IngestParsePipeline` on parse; app on classify | drain, dashboard, review | `rawIngestId`, `amount`, `currency`, `merchant`, `timestamp`, `categoryId`, `subcategoryId`, `paymentSourceId`, `unmatched`, `ambiguous`, `type`, `needsClassification`, `merchantNormalized`, `userNotes`, `shoppingItems`, `travelProvider`, `transferTo`, `classifiedBy`, `processedAt` |
...
4. **LLM gate (rules-first, optional)** — Only for `needsClassification`/`ambiguous` debits, the pipeline calls the `classifyTransaction` callable CF (Gemini). On success it sets `categoryId`/`merchantNormalized` and clears the flags; on error or missing API key it leaves the transaction for the in-app inbox (never blocks the drain). Rules + manual classify work without any LLM key.
5. **Notify (optional)** — `notifyClassification` (Firestore trigger) can send an FCM "categorize" push to `fcm_tokens` when a transaction needs input. Real APNs delivery requires a paid Apple Developer account; **sideloaded IPA does not need one** — the in-app Review/classify inbox is the primary path.
