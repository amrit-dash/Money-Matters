...
| Item | Status |
|------|--------|
| Final app icon | **Done** — teal `#1a8990`, white hub-and-spoke (₹ center, corner nodes) |
| SVG masters | **In repo** — [`docs/assets/app-icon/`](assets/app-icon/README.md) (`app-icon-master-{light,dark}.svg`) |
| iOS `AppIcon.appiconset` | **Exported** — light + `-dark` slots via `./scripts/export_app_icons.sh` |
| Launch splash | `LaunchImage.imageset` — regenerated from light master |
...
| App icon | `docs/assets/app-icon/` → `ios/Runner/Assets.xcassets/AppIcon.appiconset/` | Masters in repo; re-export via `./scripts/export_app_icons.sh` |
